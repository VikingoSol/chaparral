...
Public Class cClientes
    ...
    Public Function Agregar(ByVal pNombre As String, ByVal pRFC As String, ByVal pCalle As String, ByVal pNoInt As String, ByVal pNoExt As String, ByVal pColonia As String, ByVal pLocalidad As String, ByVal pReferencia As String, ByVal pMunicipio As String, ByVal pEstado As String, ByVal pPais As String, ByVal pCP As String, ByVal pRetIVA As Boolean, ByVal pDescFactura As Double, ByVal pEmail As String) As Integer
        If gConn.State <> ConnectionState.Open Then gConn.Open()
        Dim vCmd As New MySqlCommand("INSERT INTO clientes(rfc, nombre, calle, no_ext, no_int, colonia, localidad, referencia, municipio, estado, pais, cp, retiva, DescFactura, email) VALUE(?rfc, ?nombre, ?calle, ?no_ext, ?no_int, ?colonia, ?localidad, ?referencia, ?municipio, ?estado, ?pais, ?cp, ?riva,  ?DescFactura, ?email)", gConn)
        ...
        vCmd.Parameters.AddWithValue("?email", Trim(pEmail))
        vCmd.ExecuteNonQuery()
    ...
    ByVal pMunicipio As String, ByVal pEstado As String, ByVal pPais As String, ByVal pCP As String, ByVal pRetIva As Boolean, ByVal pDescFactura As Double, ByVal pEmail As String) As Integer
        If gConn.State <> ConnectionState.Open Then gConn.Open()
        Dim vCmd As New MySqlCommand("UPDATE clientes SET rfc=?rfc, nombre=?nombre, calle= ?calle, no_ext=?no_ext, no_int=?no_int, colonia=?colonia, localidad=?localidad, referencia=?referencia, municipio=?municipio, estado=?estado, pais=?pais, cp=?cp, retiva=?riva, DescFactura=?DescFactura, email=?email WHERE id=?id", gConn)
        vCmd.Parameters.AddWithValue("?rfc", pRFC)
        vCmd.Parameters.AddWithValue("?nombre", Trim(pNombre))
        vCmd.Parameters.AddWithValue("?calle", Trim(pCalle))
        vCmd.Parameters.AddWithValue("?no_ext", Trim(pNoExt))
    ...
    Public Function GetCliente(ByVal pId As Integer) As dCliente
        ...
        Return vCl
    End Function
    Public Function GetClienteNombre(ByVal pId As String) As dCliente
        If gConn.State <> ConnectionState.Open Then gConn.Open()
        Dim vCmd As New MySqlCommand("SELECT * FROM clientes WHERE nombre=?id", gConn)
        vCmd.Parameters.AddWithValue("?id", pId)
        Dim vAdap As New MySqlDataAdapter(vCmd)
        Dim vTabla As New DataTable
        vAdap.Fill(vTabla)
